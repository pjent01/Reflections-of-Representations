...
#    given = []
#    given.append("Given Dimension Vectors:")
#    for (A,B,C) in DimVecs:
#        given.append(f"({A}, {B}, {C})")
#    st.text("\n".join(given))
#    seq = [] 
#    seq.append("Sequence of Reflections:")
#    seq.append(sequence)
#    seq.append("  ".join([f"s{d}" for d in reversed(sequence)]))
#    st.text("\n".join(seq))
...
#            output.append(f"{arrow}  Reflection: {' '.join([f'{d}' for d in digits])}")
            output.append(f"<span style='color:{line_color}'>{arrow}  Reflection: {' '.join([f'{d}' for d in digits])}</span>")

#            output.append("\n" + "Reflection: " + " ".join([f"s{d}" for d in digits]))
#            output.append("\n" + "Reflection: " + " ".join([f"{d}" for d in digits]))
    ...
    st.markdown("  <br>  ".join(output[:max_lines]), unsafe_allow_html=True)
#    st.markdown("  \n".join(output[:max_lines]))
with col2:
#    if error != []:
#        st.write("\n Reflection not allowed:".join(error))
    for i in range(1):
        st.write("\n")
...
A, B, C, D, E = np.linalg.solve(X, Y); F_const = 1.0
...
if use_arrows:
    ...
    fig.add_trace(go.Scatter(x=[bary_to_cart(renormalize(pt))[0] for node in wrong_nodes for pt in node],
                         ...
                         mode="markers", 
                         marker=dict(color="red", size=6), name="Wrong Quiver", showlegend=True, 
    ...
    fig.add_trace(go.Scatter(x=[bary_to_cart(renormalize(pt))[0] for node in correct_nodes for pt in node],
                         ...
                         mode="markers", 
                         marker=dict(color="green", size=6), name="Correct Quiver", showlegend=True, 
...
simple_labels = ["(0,0,1)", "(1,0,0)", "(0,1,0)"]
simple_points = [BL, BR, TOP]
